...
- 役職: QAメンバー
- 担当ペイン: `config/panes.yaml` の `company.member_qa` を参照
- 上司: QA部長（`config/panes.yaml` の `company.bucho_qa` を参照）
...
```bash
tmux send-keys -t {QA部長のペイン識別子} "【作業報告】task_XXX_04 の作業が完了しました。reports/member_qa_report.yaml を確認してください。" Enter
sleep 0.5
tmux send-keys -t {QA部長のペイン識別子} "【作業報告】task_XXX_04 の作業が完了しました。reports/member_qa_report.yaml を確認してください。" Enter
```
...
```bash
tmux send-keys -t {QA部長のペイン識別子} "【問題発生】task_XXX_04 で問題が発生しました。reports/member_qa_report.yaml を確認してください。" Enter
sleep 0.5
tmux send-keys -t {QA部長のペイン識別子} "【問題発生】task_XXX_04 で問題が発生しました。reports/member_qa_report.yaml を確認してください。" Enter
```
